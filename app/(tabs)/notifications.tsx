...
export default function NotificationsScreen() {
  ...
  const processOffers = (notis: NotificationType[]): NotificationType[] => {
    ...
    const finalOffers = Array.from(map.values()).map(group => {
      return group.reduce((latest, current) => new Date(current.createdAt) > new Date(latest.createdAt) ? current : latest);
    });

    return [...others, ...finalOffers].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  };
    ...
    if (noti.type === 'offer') {
      ...
      if (isRejected || isAccepted || isSender) {
        ...
        const related = notifications.filter(n =>
          n.type === 'offer' &&
          JSON.stringify(n.cards) === JSON.stringify(noti.cards) &&
          n.amount === noti.amount &&
          n._id !== noti._id
        );
        const senderNotification = related.find(n => n.sender?._id === userId);

        route = '/judge-offer';
        query = `cards=${cardsParam}&offer=${offerParam}&friendName=${partner?.apodo}&friendId=${partner?._id}&receptorId=${noti._id}&emisorId=${senderNotification?._id}`;
      }
      ...
      {notifications.length === 0 ? (
        ...
        <ScrollView>
          {notifications.map(noti => {
            const isNew = newNotificationId === noti._id;
            if (noti.type === 'offer') {
              const isRejected = noti.message.includes('rechazada');
              const isAccepted = noti.message.includes('aceptada');
              const isSender = !noti.message.startsWith('Has recibido');
              ...
              return (
                <View key={noti._id} style={[styles.notificationBox, isNew && { borderWidth: 2, borderColor: '#6A0DAD' }]}>
            ...
            return (
              ...
              >
                <Ionicons
                  name={noti.type === 'friend_request' ? 'person-add' : 'notifications'}
                  size={24}
