// notifications.tsx  
// Versión actualizada: 2025-06-15 – Integración de apiFetch y JWT en todas las peticiones, manteniendo la función de eliminar notificaciones mediante presión prolongada.  
...
import { apiFetch } from '../../utils/apiFetch'; // helper global que agrega Authorization: Bearer <token>  
...
export default function NotificationsScreen() {  
  ...
  /**  
   * Obtiene las notificaciones del backend según userId usando apiFetch.  
   */  
  ...
  /**  
   * Marca una notificación como leída en el backend y en el estado local usando apiFetch.  
   */  
  ...
  /**  
   * Elimina una notificación (requiere confirmación) usando apiFetch.  
   */  
  ...
  const handlePress = async (noti: NotificationType) => {  
    ...
    // Finalmente, recargo la lista para reflejar cualquier cambio  
...
// Función para aceptar solicitud de amistad usando apiFetch  
async function acceptFriendRequest(noti: NotificationType) {  
  ...
  try {  
    const res = await apiFetch(`/friend-request/${noti.friendRequestId}/accept`, { method: 'POST' });  
    const data = await res.json();  
...
// Función para rechazar solicitud de amistad usando apiFetch  
async function rejectFriendRequest(noti: NotificationType) {  
  ...
  try {  
    const res = await apiFetch(`/friend-request/${noti.friendRequestId}/reject`, { method: 'POST' });  
    const data = await res.json();  
