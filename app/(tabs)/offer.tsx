...
export default function OfferScreen() {
  ...
  /**
   * Envía notificación PATCH para responder (aceptar/rechazar)
   */
  ...
  /**
   * Envía la oferta y crea notificaciones para receptor (role='receiver')
   * y emisor (role='sender'), luego muestra alerta con IDs y roles.
   */
  const sendOffer = async () => {
    try {
      ...
      // Notificar receptor → role: 'receiver'
      const resReceptor = await fetch(`${API_URL}/notifications`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId:  friendId,            // receptor
          partner: storedUser.id,       // emisor
          role:    'receiver',          // campo obligatorio
      ...
      // Notificar emisor → role: 'sender'
      const resEmisor = await fetch(`${API_URL}/notifications`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId:  storedUser.id,      // emisor
          partner: friendId,           // receptor
          role:    'sender',           // campo obligatorio
      ...
      Alert.alert(
        ...
        `IDs:\n` +
        `– Receptor (receiver): ${receptorNoti._id}\n` +
        `– Emisor   (sender):   ${emisorNoti._id}`
      );
  ...
  return (
    <View style={styles.container}>
          ...
          return (
            <View key={c.id} style={styles.cardBox}>
              ...
              <View style={styles.counterContainer}>
                ...
                <Text style={[styles.counterValue, { color: isDarkMode ? '#fff' : '#000' }]}>
                  {cur}
                </Text>
                <TouchableOpacity
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          Alert.alert('Confirmar oferta', '¿Deseas enviar la oferta?', [
